...
static DOUBLY_LIST(log_t_list);
static DOUBLY_LIST(log_domain_list);
...
void log_init(void)
{
    DOUBLY_LIST_INIT(&log_t_list);
    DOUBLY_LIST_INIT(&log_domain_list);
...
void log_final(void)
{
    ...
    doubly_list_for_each_safe(&log_t_list, saved_log, log)
        log_remove(log);
    
    mempool_destroy(&log_pool);

    doubly_list_for_each_safe(&log_domain_list, saved_domain, domain)
...
{
    log_t *log = NULL;
    doubly_list_for_each(&log_t_list, log)
    {
        switch(log->type)
        {
            case LOG_FILE_TYPE:
...
void log_remove(log_t* log)
{
    ...
    doubly_list_remove(&log_t_list, log);
...
log_domain_t *log_add_domain(const char *name, log_level_e level)
{
    ...
    doubly_list_add(&log_domain_list, domain);
    return domain;
}
...
void log_remove_domain(log_domain_t* domain)
{
    ...
    doubly_list_remove(&log_domain_list, domain);
...
log_domain_t* log_find_domain(const char *name)
{
    log_domain_t* domain = NULL;
    log_assert(name);
    doubly_list_for_each(&log_domain_list, domain)
    {
...
static log_t *add_log(log_type_e type)
{
    ...
    doubly_list_add(&log_t_list, log);
